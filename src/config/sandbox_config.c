...
static const char *TEXT_OPTION_1G = "EARTH";
static const char *TEXT_OPTION_HEAVY = "HEAVY";
...
static void createSystemEntry(MenuEntry *entry);
...
static MenuEntry *gravity_entry = NULL;
...
void CONFIG_SANDBOX_init(void) {
    ...
    createSystemEntry(planet_entry);

    gravity_entry = &config_view.entries[6];
    createGravityEntry(gravity_entry);

    enemy_entry = &config_view.entries[7];
    createEnemyEntry(enemy_entry);

    enemy_hostility_entry = &config_view.entries[8];
    createEnemyHostilityEntry(enemy_hostility_entry);

    max_enemies_entry = &config_view.entries[9];
    createMaxEnemiesEntry(max_enemies_entry);

    back_entry = &config_view.entries[10];
...
static void createGravityEntry(MenuEntry *entry) {
    ...
    entry->type = ENTRY_CONFIG;
    entry->text = TEXT_ENTRY_GRAVITY;
    entry->text_pos = MARGIN_L1;
    entry->num_options = NUM_GRAVITY_OPTIONS;
    entry->options = MEM_calloc(sizeof(ConfigOption) * entry->num_options);
    entry->current_option = 1;

    CONFIG_setOption(&entry->options[0], TEXT_OPTION_LIGHT, 0, NULL);
    CONFIG_setOption(&entry->options[1], TEXT_OPTION_1G, 1, NULL);
    CONFIG_setOption(&entry->options[2], TEXT_OPTION_HEAVY, 2, NULL);
}
...
static void expandSandboxConfig(void) {
    ...
    u8 gravity_option = gravity_entry->options[gravity_entry->current_option].value;
    sandbox_config.gravity = GRAVITY_VALUES[gravity_option];
