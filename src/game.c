...
static bool runPlanet(Planet current_planet[static 1]) {
    ...
    while (!game_over && !mission_accomplished) {
        if (!paused) {
            ...
            /*
             * Handling of dead is a complex task, that's why the following code
             * is not straight forward. If in player mode or only one player is
             * alive, dying implies restarting the enemies, but if one player
             * dies while another player is alive then the enemies stay as they
             * are but the resurrected player has immunity for a short period of
             * time.
             */
            if (!num_players_playing) {
                // Scenario 1: No players alive, though one may still have
                // lives. Wait for the explosions to extinguish and try to
